...
export default function Analytics() {
 ...
 <p className="text-white/60 mt-1">Real-time content performance and workflow metrics from your workspace.</p>
 </div>
 ...
 <span className="flex items-center text-xs font-semibold text-white/60">
 ...
 <Card>
 <CardContent className="p-6">
 <p className="text-sm font-medium text-white/60">Published Content</p>
 <div className="mt-2 flex items-baseline gap-2">
 <h2 className="text-3xl font-bold">{metrics.publishedCount}</h2>
 <span className={cn(
 "flex items-center text-xs font-semibold",
 metrics.publishedCount > 0 ? "text-emerald-600" : "text-white/50"
 )}>
 ...
 <Card>
 <CardContent className="p-6">
 <p className="text-sm font-medium text-white/60">Content Quality Score</p>
 <div className="mt-2 flex items-baseline gap-2">
 ...
 <p className="text-xs text-white/60 mt-1">Weekly breakdown of views and engagement across all posts</p>
 </div>
 </CardHeader>
 <CardContent>
 ...
 <p className="text-xs text-white/60">Pipeline execution & review efficiency</p>
 </CardHeader>
 <CardContent className="space-y-6">
 <div>
 <div className="flex items-center justify-between mb-1.5">
 <span className="text-sm font-medium text-white/70 /50">Schedule Consistency</span>
 ...
 <div>
 <div className="flex items-center justify-between mb-1.5">
 <span className="text-sm font-medium text-white/70 /50">Production Velocity</span>
 ...
 <div>
 <div className="flex items-center justify-between mb-1.5">
 <span className="text-sm font-medium text-white/70 /50">Approval Efficiency</span>
 ...
 <p className="text-xs text-white/60">Distribution of active content pieces across defined strategic pillars</p>
 </CardHeader>
 <CardContent>
 {pillarPerformance.length === 0 ? (
 <div className="text-center py-12 text-white/50 text-sm">No pillars configured yet.</div>
 ) : (
 ...
 <p className="text-xs text-white/60">Highest priority and most viewed content pieces in your workspace</p>
 </CardHeader>
 <CardContent>
 {topContent.length === 0 ? (
 <div className="text-center py-12 text-white/50 text-sm flex flex-col items-center justify-center">
 <FileText className="w-8 h-8 text-slate-300 mb-2" />
 <p>No content items yet.</p>
 <p className="text-xs text-white/50 mt-1">Create your first content piece to begin tracking analytics.</p>
 </div>
 ) : (
 ...
 <span className="font-bold text-slate-300 /70 w-4 text-center">{i + 1}</span>
 <div className="min-w-0 flex-1">
 <p className="font-medium text-sm text-white truncate">{item.title}</p>
 <div className="flex items-center gap-2 mt-1">
 <span className={cn("text-[10px] font-bold px-1.5 py-0.5 rounded uppercase", PLATFORM_COLORS[item.platform] || "bg-transparent/10 text-white/70")}>
 {item.platform}
 </span>
 <span className={cn("text-[10px] font-semibold px-1.5 py-0.5 rounded uppercase", STATUS_COLORS[item.status] || "bg-transparent/10 text-white/70")}>
 ...
 <p className="font-bold text-sm text-white ">{formatNumber(item.views || 0)}</p>
