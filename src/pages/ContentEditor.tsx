...
export default function ContentEditor() {
 ...
 {/* Header */}
 <header className="h-16 border-b border-white/10 flex items-center justify-between px-6 shrink-0 bg-transparent z-10">
 <div className="flex items-center gap-4">
 <button 
 onClick={() => navigate('/content')}
 className="p-1.5 rounded-md text-white/50 hover:text-white hover:bg-transparent/10 transition-colors"
 >
 <ArrowLeft className="w-5 h-5" />
 </button>
 
 <div className="flex items-center gap-3">
 <span className={cn("text-xs font-bold px-2 py-1 rounded-md uppercase tracking-wider", PLATFORM_COLORS[item.platform || 'Instagram'] || "bg-transparent/10 text-white/80")}>
 ...
 className="font-bold text-xl bg-transparent outline-none border-none focus:ring-0 w-64 md:w-96 placeholder:text-slate-300 dark:placeholder:text-white/80 text-white "
 />
 </div>
 </div>
 ...
 <div 
 key={collaborator.id} 
 className="w-8 h-8 rounded-full bg-indigo-100 border-2 border-white flex items-center justify-center text-indigo-700 font-bold text-xs relative group z-20"
 >
 {collaborator.avatar ? (
 ...
 <button 
 onClick={handleSave}
 className="flex items-center gap-2 btn-primary px-4 py-2 rounded-md font-medium transition-colors shadow-sm text-white"
 >
 ...
 {/* Tabs */}
 <div className="flex items-center gap-6 px-8 border-b border-white/10 bg-transparent/5 pt-2 shrink-0 overflow-x-auto hide-scrollbar">
 {TABS.map(tab => (
 <button
 key={tab}
 ...
 className="w-full px-3 py-2 glass-input outline-none focus:ring-2 focus:ring-blue-500"
 >
 ...
 <div className="space-y-2">
 <label className="text-sm font-medium text-white/80 ">Publish Date & Time</label>
 <input 
 type="datetime-local" 
 ...
 <div className="space-y-2">
 <label className="text-sm font-medium text-white/80 ">Content Pillar</label>
 <select 
 value={item.pillarId || ''}
 ...
 <div className="space-y-2">
 <label className="text-sm font-medium text-white/80 ">Topic / Core Summary</label>
 <textarea 
 value={item.description || ''}
 onChange={(e) => setItem({...item, description: e.target.value})}
 placeholder="Brief description of this content (e.g. 5 quick tips to boost engagement)..."
 className="w-full px-3 py-2 glass-input outline-none min-h-[120px] resize-y focus:ring-2 focus:ring-blue-500"
 />
 </div>
 </div>
 )}
 ...
 <p className="text-sm text-white/60">Define the objective, target audience, and key messaging pillars.</p>
 </div>
 </div>

 <div className="space-y-4">
 <div className="space-y-2">
 <label className="text-sm font-semibold text-white/80 ">Target Audience</label>
 <input 
 type="text" 
 ...
 <div className="space-y-2">
 <label className="text-sm font-semibold text-white/80 ">Key Message & Takeaway</label>
 <textarea 
 value={item.description || ''}
 onChange={(e) => setItem({...item, description: e.target.value})}
 ...
 <div className="space-y-2">
 <label className="text-sm font-semibold text-white/80 ">Call to Action (CTA)</label>
 <input 
 type="text" 
 ...
 <div className="p-4 bg-transparent/5 rounded-lg border border-white/10 flex items-start gap-3 mt-4">
 <Sparkles className="w-5 h-5 text-indigo-500 shrink-0 mt-0.5" />
 <div>
 <p className="text-sm font-medium text-white ">Need inspiration?</p>
 <p className="text-xs text-white/60 mt-1">Use the "Magic Generate" button at the top header to auto-research trending angles and draft this entire piece.</p>
 </div>
 </div>
 </div>
 </div>
 ...
 <p className="text-sm text-white/60">Draft full video scripts, carousel slide copy, or article text.</p>
 </div>
 ...
 className="w-full p-6 bg-transparent border-none outline-none min-h-[500px] resize-y text-base leading-relaxed text-slate-800 font-mono"
 />
 </CardContent>
 </Card>
 </div>
 ...
 <p className="text-sm text-white/60">Upload and organize images, videos, thumbnails, and reference documents.</p>
 </div>
 <button 
 ...
 className="px-3 py-1.5 glass-input text-sm outline-none"
 />
 <input 
 type="text" 
 ...
 className="px-3 py-1 text-xs font-medium text-white/70 hover:text-white"
 >
 Cancel
 </button>
 <button 
 ...
 {assets.map(asset => (
 <Card key={asset.id} className="overflow-hidden group border-white/10 ">
 <div className="h-36 bg-transparent/10 relative flex items-center justify-center overflow-hidden">
 {asset.type === 'image' ? (
 ...
 <p className="text-[10px] text-white/50">{formatTimeAgo(asset.uploadedAt)}</p>
 </div>
 ...
 target="_blank" 
 rel="noreferrer" 
 className="p-1.5 text-white/50 hover:text-blue-500 rounded hover:bg-transparent/10 "
 >
 <ExternalLink className="w-4 h-4" />
 </a>
 <button 
 onClick={() => handleDeleteAsset(asset.id)}
 className="p-1.5 text-white/50 hover:text-red-500 rounded hover:bg-transparent/10 "
 >
 ...
 <p className="text-sm text-white/60">Craft the post copy, hashtags, and links for publishing.</p>
 </div>
 <button 
 ...
 className="w-full p-6 bg-transparent border-none outline-none min-h-[300px] resize-y leading-relaxed text-slate-800 text-sm"
 />
 </CardContent>
 </Card>
 ...
 <p className="text-sm text-white/60">Ensure content quality and adherence to brand guidelines before approval.</p>
 </div>
 ...
 : "bg-transparent/10 text-white/70 /50 hover:bg-transparent/20"
 )}
 >
 {st.replace('_', ' ')}
 </button>
 ...
 <CardTitle className="text-sm font-semibold uppercase tracking-wider text-white/60">Pre-Publish Checklist</CardTitle>
 </CardHeader>
 ...
 <label key={check.key} className="flex items-center gap-3 p-2 rounded-md hover:bg-transparent/5 cursor-pointer text-sm">
 <input 
 type="checkbox"
 ...
 />
 <span className={checklist[check.key] ? "line-through text-white/50" : "text-white/80 "}>
 {check.label}
 </span>
 </label>
 ))}
 ...
 return (
 <div key={c.id} className={cn("p-3 rounded-lg border text-sm transition-all", c.resolved ? "bg-transparent/5 border-white/5 opacity-60" : "glass-card ")}>
 <div className="flex items-center justify-between mb-1">
 <div className="flex items-center gap-2">
 <img src={user.avatar} alt="" className="w-5 h-5 rounded-full" />
 <span className="font-semibold text-xs">{user.name}</span>
 <span className="text-[10px] text-white/50">{formatTimeAgo(c.createdAt)}</span>
 </div>
 <div className="flex items-center gap-2">
 <button 
 onClick={() => handleToggleComment(c.id)}
 className={cn("text-xs px-2 py-0.5 rounded", c.resolved ? "bg-emerald-100 text-emerald-700" : "bg-transparent/10 text-white/70 hover:bg-transparent/20")}
 >
 ...
 <p className="text-white/80 mt-1">{c.text}</p>
 </div>
 );
 })}
 </div>
 ...
 className="flex-1 px-3 py-2 glass-input text-sm outline-none focus:ring-2 focus:ring-blue-500"
 />
 <button 
 onClick={handleAddComment}
 className="px-4 py-2 btn-primary text-white rounded-md text-sm font-medium flex items-center gap-1.5"
 >
 ...
 <p className="text-sm text-white/60">Analyze your script or caption for keyword density, Flesch score, and tone.</p>
 </div>
 <button 
 ...
 <div>
 <h4 className="text-sm font-medium text-white/60 uppercase tracking-wider">Real-time Readability</h4>
 <p className="text-xs text-white/50 mt-1">{realTimeSeo.words} words, {realTimeSeo.sentences} sentences</p>
 </div>
 <span className={cn(
 ...
 <h4 className="text-sm font-medium text-white/60 uppercase tracking-wider mb-2">Real-time Keyword Density</h4>
 {realTimeSeo.keywords.length === 0 ? (
 <p className="text-sm text-white/50 italic">No keywords specified</p>
 ) : (
 <div className="space-y-2">
 {realTimeSeo.keywords.map((kw, i) => (
 <div key={i} className="flex items-center justify-between text-sm">
 <span className="font-medium text-white/80 ">{kw.keyword}</span>
 <div className="flex items-center gap-3">
 <span className="text-white/60">{kw.count} uses</span>
 <span className={cn("px-2 py-0.5 rounded text-xs font-bold", kw.density >= 1 && kw.density <= 3 ? "bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300" : "bg-transparent/10 text-white/70 /50")}>
 {kw.density}%
 </span>
 </div>
 </div>
 ...
 <div className="flex items-center justify-between">
 <h4 className="font-medium text-white/80 ">SEO Score</h4>
 <span className={cn(
 ...
 <div>
 <h5 className="text-xs font-bold text-white/60 uppercase mb-1">Keyword Analysis</h5>
 <p className="text-sm text-white/80 leading-relaxed">{seoAnalysis.keywordAnalysis}</p>
 </div>
 </CardContent>
 </Card>
 ...
 <CardContent className="p-6 space-y-4">
 <div className="flex items-center justify-between">
 <h4 className="font-medium text-white/80 ">Readability Score</h4>
 <span className={cn(
 "text-xl font-bold px-3 py-1 rounded-full",
 ...
 <div>
 <h5 className="text-xs font-bold text-white/60 uppercase mb-1">Readability Feedback</h5>
 <p className="text-sm text-white/80 leading-relaxed">{seoAnalysis.readabilityFeedback}</p>
 </div>
 </CardContent>
 </Card>

 <Card className="shadow-sm md:col-span-2 border-white/10 ">
 <CardContent className="p-6">
 <h4 className="font-medium text-white/80 mb-4">Improvement Suggestions</h4>
 <ul className="space-y-2">
 {seoAnalysis.suggestions.map((suggestion, idx) => (
 <li key={idx} className="flex items-start gap-2 text-sm text-white/70 /50 bg-transparent/5 p-3 rounded-lg border border-white/5 ">
 ...
 <div className="text-center py-12 bg-transparent/5 rounded-xl border border-dashed border-white/10 mt-6">
 <Target className="w-10 h-10 text-slate-300 mx-auto mb-3" />
 <p className="text-white/60 font-medium">Ready to analyze</p>
 <p className="text-sm text-white/50 mt-1 max-w-sm mx-auto">Enter target keywords above and click 'Run Deep AI Audit' to get actionable SEO and readability insights.</p>
 </div>
 )}
 </div>
 )}
 ...
 <div className="w-80 bg-transparent/5 border-l border-white/10 flex flex-col shrink-0">
 <div className="p-4 border-b border-white/10 font-semibold text-sm text-slate-800 ">
 Status & Workflow
 </div>
 ...
 {/* Approval Flow */}
 <div className="space-y-3">
 <h4 className="text-xs font-semibold text-white/60 uppercase tracking-wider">Quick Approval</h4>
 ...
 : "glass-card hover:bg-transparent/5 "
 )}
 >
 Needs Review
 </button>
 ...
 <h4 className="font-semibold text-white/60 uppercase tracking-wider">Meta Details</h4>
 <div className="space-y-2 text-white/70 /50">
 ...
 <span className="font-medium text-white ">{item.publishAt ? formatFriendlyDate(item.publishAt) : 'Unscheduled'}</span>
 </div>
 <div className="flex justify-between">
 <span>Words</span>
 <span className="font-medium text-white ">{realTimeSeo.words}</span>
 </div>
 </div>
 </div>
 ...
 <h4 className="text-xs font-semibold text-white/60 uppercase tracking-wider flex items-center justify-between">
 Comments
 <span className="bg-transparent/20 text-white/70 /50 px-1.5 py-0.5 rounded text-[10px]">{itemComments.length}</span>
 </h4>
 {itemComments.length === 0 ? (
 <div className="text-center py-6 text-xs text-white/50 glass-card border-dashed">
 No comments yet
 </div>
 ) : (
 <div className="space-y-2 max-h-48 overflow-y-auto">
 {itemComments.slice(0, 3).map(c => (
 <div key={c.id} className="p-2 glass-card text-xs">
 <p className="text-white/80 line-clamp-2">{c.text}</p>
 </div>
 ))}
 </div>
 )}
